# Migration Notes: ASP.NET MVC 5 (.NET Framework 4.5.1) → ASP.NET Core MVC (.NET 8)
...
EF Core, the built-in generic host, and SDK-style `net8.0` projects.
...
| Legacy (packages.config, `net451`) | Replacement (`net8.0` `PackageReference`) |
|---|---|
| `EntityFramework` 6.1.3 (`System.Data.Entity`) | `Microsoft.EntityFrameworkCore` + `Microsoft.EntityFrameworkCore.SqlServer` + `Microsoft.EntityFrameworkCore.Design` (8.x) |
...
## 9. Old-style `.csproj` → SDK-style `net8.0`
...
- Convert to SDK-style `net8.0`; add `Microsoft.NET.Test.Sdk`, `NUnit` (4.x), `NUnit3TestAdapter`, `Moq` (current).
...
- **A — Data + Service + Biz layers** (§2, §3, §7, §8, §9): SDK-style `net8.0`; EF6→EF Core; rewrite
...
- **B — Web app** (§1, §4, §5, §6, §7, §9): SDK-style `Microsoft.NET.Sdk.Web` `net8.0`;
...
- **C — Tests** (§10): SDK-style `net8.0`; NUnit4/Moq/Test.Sdk; rewrite against EF Core `SampleWebAppDb` +
...
- **A — Data/Service/Biz** (PR #24): SDK-style `net8.0`; EF Core 10; `SampleWebAppDb(DbContextOptions<>)`;
...
- **C — Tests** (PR #26): SDK-style `net8.0`; NUnit 4 / Moq 4.20 / Test.Sdk 17.11; in-memory **SQLite**
